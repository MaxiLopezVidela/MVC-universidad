...
public class ControladorVista {
    ...
    private void buscar() {
        String input = JOptionPane.showInputDialog(vista, "Ingrese ID:");
        if (input == null || input.isEmpty()) return;

        int id = Integer.parseInt(input);

        Estudiante est = control.buscarEstudiante(id);
        if (est != null) {
        ...
        ProyectoDeGrado p = control.buscarProyecto(id);
        if (p != null) {
            JOptionPane.showMessageDialog(vista,
                "PROYECTO\nID: " + p.getId() +
                "\nTítulo: "     + p.getTitulo() +
    ...
    private void editar() {
        ...
        try {
            ...
            control.editarProyecto(p);

            vista.titulo.setText(p.getTitulo());
            vista.alcance.setText(p.getAlcance());
            vista.objetivo.setText(p.getObjetivo());
            vista.plan.setText(p.getPlanDeProyecto());
            vista.costo.setText(String.valueOf(p.getCosto()));
            vista.tiempo.setText(String.valueOf(p.getTiempoDeRealizacion()));
            vista.viabilidad.setText(String.valueOf(p.getViabilidad()));

            JOptionPane.showMessageDialog(vista, "Cambios guardados.");

        } catch (NumberFormatException nfe) {
            JOptionPane.showMessageDialog(vista, "Algún campo numérico no es válido.");
        } catch (Exception ex) {
